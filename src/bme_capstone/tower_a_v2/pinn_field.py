"""PINN scaffolding for Tower A v2's physics-informed field encoder.
...
* :class:`TowerALaplaceProblem` configures the Laplace PDE with mixed boundary
  conditions given a :class:`~bme_capstone.tower_a_v2.geometry.TowerAGeometry`
  instance.
...
from .adf import DirichletGaugeWrapper
...
def _compute_normal_flux(
    ...
    """Compute ``-n   `` on the collocation points for a given patch."""
...
def build_pointwise_neumann_equation(
    ...
    """Enforce ``-n = target`` pointwise on ``patch``."""
...
class TowerALaplaceProblem(SpatialProblem):
    """PINA ``SpatialProblem`` enforcing the Tower A v2 Laplace formulation."""
    ...
    def __init__(
        ...
        self._has_dirichlet_outer = bool(self.geometry.gauge) or any(spec["type"] == "dirichlet" for spec in outer_specs.values())
    ...
    def _normalise_outer_bc(
        ...
        for patch in self.geometry.outers:
            ...
            bc_type = bc_utils.canonicalise_bc_type(spec.get("type", "neumann"))
    ...
    def _build_conditions(
        ...
        for patch in self.geometry.outers:
            if patch.kind == "gauge" or patch.metadata.get("bc_type") == "dirichlet":
                continue
            if self.geometry.gauge is not None and patch.name == self.geometry.gauge.name:
                continue
            spec = outer_specs.get(patch.name, {"type": "neumann", "value": 0.0})
            bc_type = spec["type"]
...
class TowerABasisTrainer:
    ...
    def discretise(
        ...
        elif problem.geometry.gauge is not None:
            gauge_name = problem.geometry.gauge.domain_name("gauge")
            problem.discretise_domain(n=1, mode=boundary_mode, domains=gauge_name)
    ...
    def train_basis(
        ...
        *,
        ...
        if self.geometry.gauge is not None:
            model = DirichletGaugeWrapper(model, self.geometry.gauge)
